...
async def check_mtf_confluence(exchange, symbol, base_direction):
    for tf in ['1h', '4h', '1d']:
        ...
        if direction != base_direction or call_claude_ai(symbol, tf, score) < 75: return False
    return True
                    ...
                    if final_score >= 75:
                        if await check_mtf_confluence(async_ex, symbol, direction):
                            ...
                            if order:
                                ...
                                break 
                            else: log_ghost_trade(symbol, final_score, f"Skipped: {status}")
                        else: log_ghost_trade(symbol, final_score, "Skipped: MTF Confluence Failed")
            
            if not trade_found:
                print("💤 No setups hit 75+. Waiting 60 seconds...")
                await asyncio.sleep(60)
            else: async_ex = ccxt_async.binance({'apiKey': API_KEY, 'secret': SECRET_KEY, 'enableRateLimit': True})
